...
namespace TaLib.Extension
{
    ...
    {
        delegate TicTacTec.TA.Library.Core.RetCode TA7(int startIdx, int endIdx, double[] inReal, int optInTimePeriod,
        ...
        private static TaResult RunTA7(this double[] series, TA7 handler, int period)
        {
            ...
            int startIdx = 0;
            int endIdx = seriesLength - 1;
            double[] inReal = new double[seriesLength];
            double[] outReal = new double[seriesLength];
            for (int i = 0; i < seriesLength; i++)
            {
                inReal[i] = series[seriesLength - i - 1];
            }
            ...
            double[] outRealReversed = new double[seriesLength];
            for (int i = 0; i < seriesLength; i++)
            {
                outRealReversed[i] = outReal[seriesLength - i - 1];
            }

            return new TaResult(outRealReversed, outBegIdx /*, outNbElement*/);
        }

        delegate TicTacTec.TA.Library.Core.RetCode TA7f(int startIdx, int endIdx, float[] inReal, int optInTimePeriod,
            out int outBegIdx, out int outNbElement, double[] outReal);

        private static TaResult RunTA7f(this float[] series, TA7f handler, int period)
        {
            int seriesLength = series.Length;
            int startIdx = 0;
            int endIdx = seriesLength - 1;
#if _
      float[] inReal = new float[seriesLength];
      for (int i = 0; i < seriesLength; i++) {
        inReal[i] = series[seriesLength - i - 1];
      }
#else
      ...
      for (int i = 0; i < seriesLength; i++) {
...
#else
            double[] outRealReversed = outReal;
#endif
            return new TaResult(outRealReversed, outBegIdx /*, outNbElement*/);
        }

        delegate TicTacTec.TA.Library.Core.RetCode TA12(int startIdx, int endIdx, double[] inReal, int optInTimePeriod,
            double optInNbDevUp, double optInNbDevDn,
            Core.MAType optInMAType, out int outBegIdx, out int outNBElement, double[] outRealUpperBand,
            double[] outRealMiddleBand, double[] outRealLowerBand);

        private static Tuple<double[], double[], double[]> RunTA12(this double[] series, TA12 handler, int period,
            double devUp, double devDn, Core.MAType maType)
        {
            int startIdx = 0;
            ...
            int outBegIdx;
            int outNbElement;
            handler(startIdx, endIdx, inReal, optInTimePeriod, devUp, devDn, maType, out outBegIdx, out outNbElement,
                outRealUp, outRealMid, outRealLo);
        ...
        public static TaResult SMA(this double[] series, int period)
        {
            return series.RunTA7(TicTacTec.TA.Library.Core.Sma, period);
        }

        public static TaResult SMA(this float[] series, int period)
        {
            return series.RunTA7f(TicTacTec.TA.Library.Core.Sma, period);
        }
        ...
        public static TaResult EMA(this double[] series, int period)
        {
            return series.RunTA7(TicTacTec.TA.Library.Core.Ema, period);
        }

        public static TaResult EMA(this float[] series, int period)
        {
            return series.RunTA7f(TicTacTec.TA.Library.Core.Ema, period);
        }
        ...
        public static TaResult RSI(this double[] series, int period)
        {
            return series.RunTA7(TicTacTec.TA.Library.Core.Rsi, period);
        }

        public static TaResult RSI(this float[] series, int period)
        {
            return series.RunTA7f(TicTacTec.TA.Library.Core.Rsi, period);
        }
